...
public class CodeScannerActivity extends AppCompatActivity {
    ...
    public static Intent newIntent(Context context, boolean isRegister) {
        Intent i = new Intent(context, CodeScannerActivity.class);
        i.putExtra(IS_REGISTER, isRegister);
        return i;
    }
    ...
    private void setBarCode(final SparseArray<Barcode> barCode){
        textViewBarCodeValue.post(new Runnable() {
            ...
            public void run() {
                ...
                String[] response = MainActivity.getAadharCredentialsFromQR(intentData);

                if(response==null) {
                    textViewBarCodeValue.setText("Invalid QR");
                }
                else {

                    Intent intent = new Intent();
                    intent.putExtra(AADHAR_NUMBER, response[0]);
                    intent.putExtra(NAME, response[1]);

                    if(response[2].equals("M")) {
                        intent.putExtra(GENDER, "Male");
                    }
                    else if(response[2].equals("F")){
                    ...
                    intent.putExtra(YOB, Integer.parseInt(response[3]));
                    intent.putExtra(ADDRESS, response[4]);
